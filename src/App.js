      ...
      <div style={{display:"flex",flexDirection:"column",gap:16}}>
        ...
        <div className="game-card fadeUp2" onClick={()=>!stakeFrozen&&goTo("game-stake")} style={{background:"#0e0e0e",borderRadius:20,padding:"28px 24px",cursor:stakeFrozen?"not-allowed":"pointer",opacity:stakeFrozen?0.5:1,transition:"all 0.2s",display:"flex",gap:20,alignItems:"center"}}>
          ...
          <div style={{flex:1}}>
            ...
            <p style={{fontSize:"0.88rem",color:"rgba(255,255,255,0.55)",lineHeight:1.6,fontWeight:300}}>{stakeFrozen?"Temporarily unavailable — check back soon.":"Pick topics, difficulty and question count. Bet real money. Winner takes 95%."}</p>
          </div>
...
const GameStake = ({ goTo, currency, is18Plus, frozen=false, joinCode, userId, userName }) => {
  ...
  const finishGame = async (finalPlayers) => {
    ...
    if (session.stake_amount > 0) {
      const pot = Number(session.stake_amount) * finalPlayers.length * 0.95;
        ...
        {session && !error && (
          mode==="stake" && !ageKnownAdult ? (
            is18Plus===false ? (
              ...
              <>
                <p style={{color:"rgba(255,255,255,0.5)",fontSize:"0.9rem",lineHeight:1.7,marginBottom:20}}>This game involves staking {cur.symbol}{session.stake_amount} of real money. Winner takes 95% of the pot.</p>
      ...
      <div style={{maxWidth:480,margin:"0 auto",padding:"48px 24px",textAlign:"center"}}>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:14}}>
          ...
          <button onClick={()=>goToSetup("stake")} style={{padding:"22px",borderRadius:18,border:"1.5px solid #ff5c3a",background:"#fff8f0",cursor:"pointer",textAlign:"left"}}>
            ...
            <p style={{fontSize:"0.85rem",color:"#888",fontWeight:300}}>Everyone stakes the same amount. Winner takes 95% of the pot. Requires an account and 18+.</p>
          </button>
        </div>
      </div>
    </div>
      ...
      return (
        <div style={{minHeight:"100vh",background:"#0e0e0e",display:"flex",alignItems:"center",justifyContent:"center",padding:"40px 24px"}}>
          <div className="popIn" style={{maxWidth:400,textAlign:"center"}}>
            <Icons.lock s={28} c="#ff5c3a"/>
            <h2 className="syne" style={{fontSize:"1.8rem",fontWeight:800,color:"white",marginTop:20,marginBottom:12}}>18+ Only</h2>
            <p style={{color:"rgba(255,255,255,0.5)",fontSize:"0.9rem",lineHeight:1.7,marginBottom:32,fontWeight:300}}>Stake & Win involves real money. Highest score wins 95% of the pot.</p>
          ...
          {mode==="stake" && (
            <div style={{background:"white",borderRadius:16,padding:"20px",border:"1px solid rgba(0,0,0,0.08)",marginBottom:16}}>
              ...
              {stakeAmount>0&&<div style={{marginTop:12,padding:"10px 14px",background:"#f0efec",borderRadius:10,fontSize:"0.83rem",color:"#555"}}>
                <Icons.trophy s={14} c="#0e0e0e"/> Winner receives up to: <strong>{cur.symbol}{Math.round(Number(stakeAmount)*expectedPlayers*0.95).toLocaleString()}</strong>
              </div>}
            </div>
          )}
        ...
        <div style={{maxWidth:480,margin:"0 auto",padding:"40px 24px"}}>
          <div className="popIn" style={{textAlign:"center",marginBottom:28}}>
            ...
            {!isTie && session.stake_amount>0 && (
              <p style={{color:"#888",fontWeight:300,fontSize:"0.9rem"}}>{sorted[0].display_name} received <strong style={{color:"#16a34a"}}>{cur.symbol}{Math.round(session.stake_amount*players.length*0.95).toLocaleString()}</strong> in their wallet.</p>
            )}
            {isTie && session.stake_amount>0 && (
  ...
  if(sec==="help") return (
    <SubPage title="Help & FAQ">
      {[
        ...
        {q:"How does Stake & Win work?",a:"All players stake the same amount. Answer trivia questions based on your chosen topics and difficulty. Highest score wins 95% of the total pot."},
